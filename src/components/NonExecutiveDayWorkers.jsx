...
const NonExecutiveDayWorkers = () => {
  ...
  const fetchPowerPlants = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('http://localhost:4000/api/non-executive-day-workers/power-plants');
  ...
  // Fetch summary data, including OT and DOT hours
  const fetchSummary = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:4000/api/non-executive-day-workers/summary', {
  ...
  // Handle approve button to save summary with OT and DOT hours
  const handleApprove = async () => {
    ...
    try {
      ...
      await axios.post(
        'http://localhost:4000/api/non-executive-day-workers/approve',
  ...
  // Fetch summary when plantId, year, or month changes
  useEffect(() => {
    if (plantId) {
      fetchSummary();
    }
  ...
  return (
    <Container className="non-executive-day-workers">
      <Typography variant="h4" component="h1" className="title">
        Non-Executive Day Workers Summary
      </Typography>
              ...
              <TableBody>
                {summary.map((row) => (
                  <TableRow key={row.employee_id}>
                    ...
                    <TableCell>{row.dot_hours}</TableCell>
...
export default NonExecutiveDayWorkers;